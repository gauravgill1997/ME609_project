...
function [l,F] = Bounding_Search(x0,s1,j,f,R)   % Bounding Phase and Newton Method for finding alfa
...
[l,F]=Newton_rapson(y1,y3,x0,s1,j,R);          % to get better value alfa we apply newton  method after bounding phase
 
end
function [alfa,F]=Newton_rapson(y1,y3,x0,s1,j,f,R) 
%  fprintf('newton rapson  Method');
 if(y1<y3)
     y1=y1;            % swaping y1 and y3 to get get lower limit and upper limit for newton method
 else
     t=y1;
     y1=y3;
     y3=t;
 ...
 y4=(y1+y3)/2;
 while true
 delta_x = .1; e = 10^-3;           %setting termination condition

 y5=y4+delta_x;
 y6=y4-delta_x;
 f1 = Objective_Fun(y4,x0,s1,j,R);
 f2 = Objective_Fun(y5,x0,s1,j,R);
 f3 = Objective_Fun(y6,x0,s1,j,R);
 f4 =(f2-f3)/2*delta_x;
 f5= (f1-2*f3+f4)/(delta_x*delta_x);
 y7=y4-f4/f5;
 feval=feval+3;
 if abs(f4)<e
     z=y7;
